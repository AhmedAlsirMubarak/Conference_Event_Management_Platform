...
@section('title', $member->name . ' | Strategic Committee')
...
@section('content')
    ...
    <div class="w-full max-w-4xl">
        ...
        <div class="bg-white rounded-xl border border-gray-200 overflow-hidden shadow-sm">
            ...
            <div class="bg-linear-to-r from-cyan-50 to-blue-50 px-8 py-8">
                <div class="flex items-start gap-6">
                    <!-- Photo -->
                    <div class="shrink-0">
                        <div
                            class="w-32 h-32 bg-white rounded-xl border-2 border-cyan-200 flex items-center justify-center overflow-hidden shadow-md">
                            @if ($member->photo)
                    ...
                    <div class="flex-1">
                        ...
                        @if ($member->title)
                            <p class="text-lg text-cyan-600 font-semibold mb-1">{{ $member->title }}</p>
                        @endif
                        @if ($member->company)
                    ...
                    @if ($member->logo)
                        <div class="shrink-0 text-center">
                            ...
                            <div class="w-24 h-24 bg-white rounded-lg border border-gray-200 flex items-center justify-center">
                                <img src="/storage/{{ $member->logo }}" alt="Logo" class="max-w-[90%] h-auto object-contain">
                            </div>
                        </div>
                    @endif
                </div>
