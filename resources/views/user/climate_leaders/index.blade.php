...
@section('title', 'Climate Leaders | User Dashboard')
@section('header-title', 'Climate Leaders Network')
@section('header-subtitle', 'Browse and connect with climate leaders')
...
@section('content')
    <div class="flex flex-col">
        ...
        <div class="mb-6 flex items-center justify-between">
            <div>
                <h3 class="text-lg font-semibold text-gray-900">Climate Leaders Network</h3>
                <p class="text-sm text-gray-500 mt-1">Total: {{ count($climateLeaders) }} leaders</p>
            </div>
            <div class="flex items-center gap-3">
                <a href="{{ route('user.climate-leaders.export') }}"
        ...
        <!-- Search Bar -->
        <div class="mb-6">
            <form method="GET" action="{{ route('user.climate-leaders.index') }}" class="flex flex-col sm:flex-row gap-2">
                            ...
                            @foreach ($climateLeaders as $leader)
                                <tr class="hover:bg-gray-50 transition-colors">
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <div class="flex items-center gap-3">
                                            <div
                                                class="w-10 h-10 bg-gradient-to-br from-green-400 to-green-600 rounded-full flex items-center justify-center text-white font-bold text-sm">
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <a href="mailto:{{ $leader->email }}" class="text-sm text-green-600 hover:text-green-800">
                                            {{ $leader->email }}
                                        </a>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        ...
                                        <div class="flex items-center gap-1">
                                            @if ($leader->notes)
                                                ...
                                                <button type="button"
                                                    ...
                                                    class="inline-flex items-center gap-1 px-2 py-1 bg-green-100 text-green-700 hover:bg-green-200 rounded text-xs font-medium transition-colors whitespace-nowrap">
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-right">
                                        <div class="flex items-center justify-end gap-2">
                                            <a href="{{ route('user.climate-leaders.show', $leader->id) }}"
                                                class="inline-flex items-center gap-1 px-3 py-1 bg-green-100 hover:bg-green-200 text-green-700 rounded text-sm font-medium transition-colors">
                                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                            ...
                                            @if ($leader->linkedin_profile)
                                                <a href="{{ $leader->linkedin_profile }}" target="_blank"
