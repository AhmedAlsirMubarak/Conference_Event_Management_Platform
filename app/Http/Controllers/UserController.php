    ...
    public function globalSearch(Request $request)
    {
        try {
            ...
            $contactResults = contacts::where('Name', 'like', "%{$query}%")
                ...
                ->map(function ($item) {
                    $item->type = 'Contact';
                    $item->route = route('user.contacts.show', $item->id);
            ...
            $sponsorResults = Sponsor::where('name', 'like', "%{$query}%")
                ...
                ->map(function ($item) {
                    $item->type = 'Sponsor';
                    $item->route = route('user.sponsors.show', $item->id);
            ...
            $speakerResults = StrategicSpeaker::where('name', 'like', "%{$query}%")
                ...
                ->map(function ($item) {
                    ...
                    $item->route = route('user.strategic_speakers.show', $item->id);
            ...
            $techSpeakerResults = TechnicalSpeaker::where('name', 'like', "%{$query}%")
                ...
                ->map(function ($item) {
                    ...
                    $item->route = route('user.technical_speakers.show', $item->id);
                    $item->detail = $item->company ?? $item->title;
                    $item->Name = $item->name;
                    return $item;
                });
            ...
            $committeeResults = StrategicCommittee::where('name', 'like', "%{$query}%")
                ...
                ->map(function ($item) {
                    ...
                    $item->route = route('user.strategic_committees.show', $item->id);
                    $item->detail = $item->Company ?? $item->title;
                    $item->Name = $item->name;
                    return $item;
                });
            ...
            $techCommitteeResults = TechnicalCommittee::where('name', 'like', "%{$query}%")
                ...
                ->map(function ($item) {
                    ...
                    $item->route = route('user.technical_committees.show', $item->id);
                    $item->detail = $item->Company ?? $item->title;
                    $item->Name = $item->name;
                    return $item;
                });
            ...
            return view('user.search-results', compact('query', 'searchResults'));
        } catch (\Exception $e) {
            return redirect(route('user.dashboard'))->with('error', 'Search error: ' . $e->getMessage());
        }
    }
