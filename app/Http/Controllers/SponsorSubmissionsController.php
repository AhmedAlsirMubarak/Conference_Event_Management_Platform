...
class SponsorSubmissionsController extends Controller
{
    /**
     * Display a listing of the resource (Admin).
     */
    public function getAllSponsorSubmissions()
    {
        try {
            $submissions = SponsorSubmission::all()->sortBy('created_at')->reverse();
            return view('admin.sponsorsubmissions.index', compact('submissions'));
        } catch (\Exception $e) {
            return view('admin.sponsorsubmissions.index', ['submissions' => collect()]); 
        }   
    }
    ...
    public function userIndex()
    {
        try {
            $submissions = SponsorSubmission::all()->sortBy('created_at')->reverse();
            return view('user.sponsorsubmissions.index', compact('submissions'));
        } catch (\Exception $e) {
            return view('user.sponsorsubmissions.index', ['submissions' => collect()]); 
        }   
    }
    ...
    public function show($id)
    {
        try {
            ...
            return view('admin.sponsorsubmissions.show', compact('submission'));
        } catch (\Exception $e) {
            return redirect(route('sponsor-submissions.index'))->with('error', 'Submission not found.');
        }
    }
    ...
    public function userShow($id)
    {
        try {
            $submission = SponsorSubmission::findOrFail($id);
            return view('user.sponsorsubmissions.show', compact('submission'));
        } catch (\Exception $e) {
            return redirect(route('user.sponsor-submissions.index'))->with('error', 'Submission not found.');
        }
    }
    ...
    public function create()
    {
        ...
        try {
            Log::info('Sponsor submission validation passed', $validated);
            $submission = SponsorSubmission::create($validated);
            ...
            Log::info('Sponsor submission created with notifications', ['submission_id' => $submission->id]);
            
            return redirect()->back()->with('success', 'Your sponsor submission has been received successfully.');
        } catch (\Exception $e) {
    ...
    public function exportExcel()
    {
        try {
            ...
            return response($csvContent)
                ...
                ->header('Content-Disposition', 'attachment; filename="sponsor_submissions.csv"');
        } catch (\Exception $e) {
        ...
        try {
            ...
            if (empty($query)) {
                ...
                if ($user && $user->role === 'admin') {
                    return redirect(route('sponsor-submissions.index'));
                } else {
                    return redirect(route('user.sponsor-submissions.index'));
                }
            }
            
            $submissions = SponsorSubmission::where('contact_person', 'like', "%{$query}%")
            ...
            if ($user && $user->role === 'admin') {
                return view('admin.sponsorsubmissions.index', compact('submissions'))->with('searchQuery', $query);
            } else {
                return view('user.sponsorsubmissions.index', compact('submissions'))->with('searchQuery', $query);
            }
        } catch (\Exception $e) {
            $user = Auth::user();
            if ($user && $user->role === 'admin') {
                return redirect(route('sponsor-submissions.index'))->with('error', 'Search error: ' . $e->getMessage());
            } else {
                return redirect(route('user.sponsor-submissions.index'))->with('error', 'Search error: ' . $e->getMessage());
            }
        }   
    }

    /**
     * Delete a sponsor submission
     */
    ...
    {       
        try {
            ...
            $submission = SponsorSubmission::findOrFail($id);
            $submission->delete();
            Log::info('Sponsor submission deleted', ['submission_id' => $id, 'deleted_by' => $user->id]);

            return redirect(route('sponsor-submissions.index'))->with('success', 'Submission deleted successfully!');
        } catch (\Exception $e) {
            Log::error('Error deleting sponsor submission: ' . $e->getMessage());
            return redirect()->back()->with('error', 'Unable to delete submission: ' . $e->getMessage());
        }
    }   

    /**
     * Update SponsorSubmission notes (admin)
     */ 
    ...
    {   
        try {
            ...
            $submission->save();
            Log::info('Sponsor submission notes updated', ['submission_id' => $id, 'updated_by' => $user->id]);

            return redirect()->back()->with('success', 'Submission notes updated successfully!');
        } catch (\Exception $e) {
            Log::error('Error updating sponsor submission notes: ' . $e->getMessage());
